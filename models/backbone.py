...
def build_backbone(args):
    ...
    return_interm_layers = (args.num_feature_levels > 1)  # default 1
    if args.backbone == 'resnet101':
        backbone = Backbone(args.backbone, train_backbone, return_interm_layers, args)
        model = Joiner(backbone, position_embedding)
    else: # args.backbone == 'CLIP':
        backbone, preprocess = clip.load("RN101", device="cuda" if torch.cuda.is_available() else "cpu") # ~/.cache/clip
        model = MM_Joiner(backbone, position_embedding)
    return model
