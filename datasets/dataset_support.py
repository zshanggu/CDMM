...
def build_support_dataset(image_set, args):
    if not args.fewshot_finetune:
        ...
        if args.dataset_file == 'uodd':
            root = Path('data/UODD/annotations')
            img_folder = root.parent / "images"
            ids = uodd_class_ids
            ann_file = root / f'{args.num_shots}_shot.json'
            return SupportDataset(img_folder, str(ann_file),
                                  activatedClassIds=ids,
                                  transforms=make_support_transforms(),
                                  cache_mode=args.cache_mode,
        ...
        if args.dataset_file == 'artaxor':
            root = Path('data/ArTaxOr/annotations')
            img_folder = root.parent / "images"
        ...
        if args.dataset_file == 'dior':
            root = Path('data/DIOR/annotations')
            img_folder = root.parent / "images"
            ids = dior_class_ids
            ann_file = root / f'{args.num_shots}_shot.json'
            return SupportDataset(img_folder, str(ann_file),
